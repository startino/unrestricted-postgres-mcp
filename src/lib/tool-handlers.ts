          ...
          text: JSON.stringify(
            {
              ...
              connection_status: {
                transaction_id: txStatusResult.rows[0]?.current_tx_id || null,
                in_transaction: txStatusResult.rows[0]?.in_transaction || false,
                aborted_state: abortedState,
                database_name: sessionResult.rows[0]?.database_name,
                user_name: sessionResult.rows[0]?.user_name,
                current_schema: sessionResult.rows[0]?.current_schema,
                postgres_version: sessionResult.rows[0]?.postgres_version,
              },
            },
            null,
            2,
      ...
      content: [
        {
          ...
          text: JSON.stringify(
            {
              status: "error",
              message: `Error getting connection status: ${error instanceof Error ? error.message : String(error)}`,
            },
            null,
            2,
          ),
